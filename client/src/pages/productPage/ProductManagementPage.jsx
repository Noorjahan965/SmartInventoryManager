			...
			<div className="overflow-x-auto overflow-y-auto max-h-[70vh] rounded-xl shadow-lg border border-slate-300">
				<table className="min-w-full text-left text-sm">
					...
					<tbody className="text-slate-800 bg-white">
						...
						{products.map((p) => (
							<tr key={p.sno} className="border-b hover:bg-blue-50/30 transition">
								<td className="px-4 py-3 text-slate-500 font-medium">{p.sno}</td>
								...
								<td className="px-4 py-3">
									<span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${p.currentQuantity <= p.minQuantity
								...
								<td className="px-4 py-3 font-medium text-slate-600">${p.cp}</td>

								{/* 3. COLORED SP PRICE */}
								<td className="px-4 py-3 font-bold text-blue-600">${p.sp}</td>

								<td className="px-4 py-3 text-slate-500 font-medium">{p.minQuantity}</td>
								...
								<td className="px-4 py-3">
									<div className="flex gap-1 flex-wrap max-w-150px">
										{p.variant?.length ? p.variant.map((v, i) => (
											<span key={i} className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded border border-slate-200 text-[10px] font-semibold uppercase tracking-wider">
												{v}
											</span>
								...
								<td className="px-4 py-3">
									<button
										...
										className="cursor-pointer hover:opacity-70 transition p-1 bg-white border border-slate-200 rounded shadow-sm"
									>
										<BarCode value={p.sno} width={0.8} height={18} />
									</button>
								</td>
								...
								<td className="px-4 py-3">
									<div className="flex justify-center gap-4">
										...
										<button
											...
											className="text-red-500 hover:bg-red-100 p-2 rounded-full transition cursor-pointer"
